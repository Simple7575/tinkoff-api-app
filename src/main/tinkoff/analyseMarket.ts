...
// types
import { type TickerAndClasscode, type IntervalTinkoff, type TIntervals } from '../../types/tinkoff'
...
export const analyseMarket = async (
  scheduleIntervals: IntervalTinkoff,
  ...
  for (const ticker of tickersAndClasscodes) {
    try {
      ...
      const fiveMinRes = await analyse(ticker, '5m', scheduleIntervals)
      console.log('-------------------------------5M run')
      const hourRes = await analyse(ticker, '1h', scheduleIntervals)
      console.log('-------------------------------1H run')
      // const dayRes = await day(ticker, "1d", scheduleIntervals);
      // console.log("-------------------------------1D run");
      ...
      if (fiveMinRes === BUY && hourRes === BUY) {
        ...
        await sendMessage(
          `${scheduleIntervals} ${ticker.ticker} ${BUY}  Vashe Soobshenie zdes mojno propisat`
        )
        // Sell
      } else if (fiveMinRes === SELL && hourRes === SELL) {
        // otpravlyaem soobshenie sell
        await sendMessage(
          `${scheduleIntervals} ${ticker.ticker} ${SELL}  Vashe Soobshenie zdes mojno propisat`
        )
      }
    } catch (error) {
      console.error(error)
